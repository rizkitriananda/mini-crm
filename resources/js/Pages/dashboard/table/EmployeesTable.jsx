...
export default function EmployeesTable() {
    ...
    const [totalEmployees, setTotalEmployees] = useState(0); // Total items
    const [totalPages, setTotalPages] = useState(0); // Total pages

    const theadTable = ["Name", "Company", "Division", "Email", "Phone"];
    ...
    const fetch = async (page) => {
        try {
            ...
            setTotalEmployees(data.employees.total);
            setTotalPages(data.employees.last_page); // Total pages
            setPerPage(data.employees.per_page); // Items per page
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            notify("Employee successfully deleted");
            await fetch();
        } catch (error) {
                        ...
                        <span className="font-semibold text-gray-900 dark:text-white">
                            {" " +
                                (employees.length > 0
                                    ...
                                    : 0) +
                                " to " +
                                (currentPage * perPage > totalEmployees
                                    ? totalEmployees
                                    : currentPage * perPage) +
                                " "}
                        </span>
